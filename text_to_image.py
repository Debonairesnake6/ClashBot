...
class TableToImage:
    ...
    def __init__(self, table: str, table_colour: list = False, original_rows: list = False):
        """
        Create an image from the text table given to it
    ...
    def colour_specific_word(self):
        ...
        if self.colour != '':
            for x in range(5):
                ...
                if self.table.split('\n')[3 + self.row_cnt].count(self.original_rows[self.row_cnt][self.column_cnt]) \
                        == 1:
                    x_position = self.table.split('\n')[3 + self.row_cnt].index(self.original_rows
                                                                                [self.row_cnt][self.column_cnt])

                # If the word appears multiple times
                else:
                    x_position = [champ for champ in range(len(self.table.split('\n')[3 + self.row_cnt]))
                                  if self.table.split('\n')[3 + self.row_cnt]
                                  .startswith(self.original_rows[self.row_cnt][self.column_cnt],
                                              champ)][self.processed.count(self.original_rows
                                                                           [self.row_cnt][self.column_cnt])]

                # Draw the word
                self.draw.text(((x_position * 12) + 12, ((self.row_cnt + 3) * 21) + 12),
                               self.original_rows[self.row_cnt][self.column_cnt], font=self.font,
                               fill=self.colours[self.colour])
        self.processed.append(self.original_rows[self.row_cnt][self.column_cnt])
